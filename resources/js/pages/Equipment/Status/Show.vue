<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
              ...
              <div class="bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg p-6">
                <div class="text-center">
                  <div class="flex justify-center mb-3">
                    <div :class="`w-20 h-20 rounded-full flex items-center justify-center ${getStatusBgColor(equipment.operational_status)}`">
                      <Icon :icon="getStatusIcon(equipment.operational_status)" class="w-8 h-8 text-white" />
                    </div>
                  </div>
                  <h4 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">
                    {{ $t('equipment.status.current_status') }}
                  </h4>
                  <el-tag :type="getStatusColor(equipment.operational_status)" size="large">
                    {{ $t(`equipment.status.operational_status.${equipment.operational_status}`) }}
                  </el-tag>
                  <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">
                    {{ $t('equipment.status.reading_time') }}: {{ formatDateTime(equipment.reading_time) }}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <!-- Dynamic Parameters -->
              <div 
                v-for="(value, key) in equipment.parameters" 
                :key="key"
                class="text-center p-4 bg-gray-50 dark:bg-gray-900/20 rounded-lg"
              >
                <div class="flex justify-center mb-2">
                  <Icon icon="heroicons:cog-6-tooth" class="w-8 h-8 text-gray-500" />
                </div>
                <p class="text-2xl font-bold text-gray-900 dark:text-white">
                  {{ value || '0' }}
                </p>
      ...
      <!-- Error State -->
      <!-- <div v-else-if="!equipment" class="text-center py-12">
...
const fetchData = async () => {
  try {
    ...
    equipment.value = {
      ...
      parameters: {
        load_percentage: 85,
        temperature_reading: 485,
        pressure_reading: 25.5,
        vibration_level: 2.1
      },
      current_readings: {

...
const fetchData = async () => {
    try {
        ...
        console.error('Error fetching gas buyer:', error);
        isLoading.value = false;
    }
};
...
const selectChange = (newValue) => {
    ...
    const selectedGasBuyer = dataList.value.find(item => item.id === newValue);
    if (selectedGasBuyer) {
        emit('data', selectedGasBuyer);
    }
};
</script>